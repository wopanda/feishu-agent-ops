...
from interaction_contract import build_interaction_contract  # type: ignore
from render_user_reply import render_user_reply, assert_no_internal_fields  # type: ignore
...
def run_pipeline(request_path: str, config_path: str) -> Dict[str, Any]:
    ...
    interaction = build_interaction_contract(normalized, observed, desired, validation, patch_preview)
    user_reply = render_user_reply(interaction)
    assert_no_internal_fields(user_reply)
...
def main() -> None:
    ...
    if args.output_dir:
        ...
        maybe_write_json(outdir / "interaction.json", result["interaction"])
        maybe_write_json(outdir / "user-reply.json", result["userReply"])
