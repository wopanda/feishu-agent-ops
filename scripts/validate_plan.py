...
def validate_plan(request: Dict[str, Any], desired: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if dup_agents:
        add_issue(
            issues, "P1", "duplicate_agent_ids", "plannedAgents 中存在重复 agentId",
            f"duplicate agent ids = {dup_agents}",
            "修正 agentId 生成规则，确保 plannedAgents 中每个 id 唯一。",
        )

    dup_accounts = find_duplicates([a.get("accountId") for a in planned_accounts])
    if dup_accounts:
        add_issue(
            issues, "P1", "duplicate_account_ids", "plannedAccounts 中存在重复 accountId",
            f"duplicate account ids = {dup_accounts}",
            "修正 accountId，确保新增账号唯一。",
        )
    ...
    if dup_bindings:
        add_issue(
            issues, "P1", "duplicate_bindings", "plannedBindings 中存在重复 binding",
            f"duplicate bindings = {dup_bindings}",
            "去重 plannedBindings，避免同一 routing 被重复写入。",
        )
    ...
    if routing_mode == "group":
        ...
        if missing_chat:
            add_issue(
                issues, "P1", "group_binding_missing_chat_id", "群聊路由缺少 chatId",
                f"group bindings without chatId = {len(missing_chat)}",
                "为 group routing 提供 chatId，否则无法生成稳定群聊绑定。",
            )
    ...
    if agent_mode == "bind-existing":
        ...
        if missing_agent:
            add_issue(
                issues, "P1", "bind_existing_missing_agent_id", "bind-existing 模式下存在缺失 agentId 的 binding",
                f"bindings missing agentId = {len(missing_agent)}",
                "bind-existing 模式必须明确指定 agentId。",
            )

    if scenario in {"bootstrap", "expand"} and not planned_bindings:
        add_issue(
            issues, "P2", "no_planned_bindings", "当前预览没有生成任何 binding",
            "plannedBindings is empty",
            "检查 request 与 observed-state 是否导致预览被全部判定为已存在。",
        )

    request_bots = request.get("bots") or []
    if scenario in {"bootstrap", "expand"} and request_bots and not planned_accounts:
        add_issue(
            issues, "P2", "no_planned_accounts", "当前预览没有生成任何 account",
            "plannedAccounts is empty while request contains bots",
            "确认这批 bot 是否都已存在；若只是做预览，应区分空白现场与真实现场。",
        )
