...
description: 当用户要把飞书机器人接入 OpenClaw，或排查多 Agent / 多机器人异常时使用。前台只保留两个用户入口：添加机器人、排查问题。用户只提供必要信息，系统先扫描当前环境，再给接入方案预览或排查结论。
---
...
> 用户只需要把需求和必要信息说出来；先扫描当前环境，再给最合适的方案和预览。
